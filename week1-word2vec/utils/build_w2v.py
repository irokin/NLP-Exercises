...
def build(train_x_seg_path, test_y_seg_path, test_seg_path, out_path=None, sentence_path='',
    ...
    """
    # With LineSentence
    w2v = Word2Vec(LineSentence(sentence_path),
        size=256,
        sg=1,
        workers=6,
        min_count=min_count
        )
    # w2v = Word2Vec(list(map(lambda sentence: sentence.split(), sentences)), 
    #     size=256, 
    #     sg=1, 
    #     workers=6,
    #     min_count=min_count)
...
if __name__ == '__main__':
    build('{}/datasets/train_set.seg_x.txt'.format(BASE_DIR),
          ...
          out_path='{}/datasets/word2vec.pickle'.format(BASE_DIR),

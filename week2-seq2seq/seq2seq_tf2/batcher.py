...
class Vocab:
    def __init__(self, vocab_file, max_size):
        """Read `max_size` words from vocabulary at `vocab_file`.

        Args:
            vocab_file ([str]): path to the vocabulary file.
            max_size ([int]): max number of words in the vocabulary to read.
        """
...
def article_to_ids(article_words, vocab):
    """ Map the sequence of words in an article by consulting the `vocab` 
    to a list of word ids.

    Args:
        article_words ([type]): a sequence of words in the article.
        vocab ([type]): a vocab.

    Returns:
        ids: a sequence of word ids
        oovs: a list of out-of-vocabulary terms.
    """
    ids = []
    oovs = []
...
def example_generator(vocab, train_x_path, train_y_path, test_x_path, max_enc_len, max_dec_len, mode, batch_size):
    """A generator produces annotated (article, abstract) instances.

    Args:
        vocab ([type]): vocab with additional decoding tokens.
        train_x_path ([type]): path to segmented training articles.
        train_y_path ([type]): path to segmented training abstracts.
        test_x_path ([type]): path to segmented test articles.
        max_enc_len ([type]): articles are truncated to this length, which corresponds to the length
        of the encoders.
        max_dec_len ([type]): abstract are truncated to this length
        mode ([type]): mode
        batch_size ([type]): number of examples.
    ...
    if mode == "train":
        ...
        for raw_record in train_dataset:
            ...
            # the max_enc_len cannot be greater than the length of shortest article.
